...
class Env(gym.Env):
    ...
    def load_config(self):
        with open(os.path.join(pathlib.Path().resolve(), f"training_envs/configs/centering0.json"), "r") as fp:
    ...
    def step(self, action, dt=1/10, convert_action=False):#dt=1/5, convert_action=True):
        ...
        if self.step_count >= self.MAX_STEPS_COUNT:
            return state, -10000, True, {}#return state, -200, True, {}
        return state, rwd if not done else -10, done, {}#return state, (rwd + 1)/2 if not done else -100, done, {}
